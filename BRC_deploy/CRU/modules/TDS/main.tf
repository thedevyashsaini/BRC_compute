...
resource "azurerm_network_interface" "push_worker_1_nic" {
  name                = "${var.worker_vm_name}-1-nic"
  resource_group_name = azurerm_resource_group.rg.name
  location            = azurerm_resource_group.rg.location
...
resource "azurerm_linux_virtual_machine" "push_worker_1" {
  name                = "${var.worker_vm_name}-1"
  resource_group_name = azurerm_resource_group.rg.name
  location            = azurerm_resource_group.rg.location
  size                = var.worker_vm_size
  admin_username      = var.admin_username
  network_interface_ids = [azurerm_network_interface.push_worker_1_nic.id]
  ...
  provisioner "local-exec" {
    command = "cp ${var.worker_env_path} ${var.worker_env_modified_path} && echo 'RABBITMQ_URL=amqp://${var.rabbitmq_user}:${var.rabbitmq_password}@${var.controller_public_ip}:5672\\nQUEUE_NAME=${var.queue_name}\\nWORKER_NAME=${var.worker-2-name}' | cat - ${var.worker_env_modified_path} > temp && mv temp ${var.worker_env_modified_path}"
  }
  ...
  connection {
    ...
    user        = var.admin_username
    private_key = file(var.ssh_private_key_path)
    host        = azurerm_network_interface.push_worker_1_nic.private_ip_address
...
resource "azurerm_network_interface" "push_worker_2_nic" {
  name                = "${var.worker_vm_name}-2-nic"
  resource_group_name = azurerm_resource_group.rg.name
  location            = azurerm_resource_group.rg.location
...
resource "azurerm_linux_virtual_machine" "push_worker_2" {
  name                = "${var.worker_vm_name}-2"
  resource_group_name = azurerm_resource_group.rg.name
  location            = azurerm_resource_group.rg.location
  size                = var.worker_vm_size
  admin_username      = var.admin_username
  network_interface_ids = [azurerm_network_interface.push_worker_2_nic.id]
  ...
  provisioner "local-exec" {
    command = "cp ${var.worker_env_path} ${var.worker_env_modified_path} && echo 'RABBITMQ_URL=amqp://${var.rabbitmq_user}:${var.rabbitmq_password}@${var.controller_public_ip}:5672\\nQUEUE_NAME=${var.queue_name}\\nWORKER_NAME=${var.worker-3-name}' | cat - ${var.worker_env_modified_path} > temp && mv temp ${var.worker_env_modified_path}"
  }
  ...
  connection {
    ...
    user        = var.admin_username
    private_key = file(var.ssh_private_key_path)
    host        = azurerm_network_interface.push_worker_2_nic.private_ip_address
    bastion_host = azurerm_public_ip.master_push_worker_ip.ip_address
    bastion_user = var.admin_username
    bastion_private_key = file(var.ssh_private_key_path)
  }
